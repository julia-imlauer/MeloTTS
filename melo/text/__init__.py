...
def cleaned_text_to_sequence(cleaned_text, tones, language, symbol_to_id=None):
    ...
    try:
      ...
      #print(f"Looking up symbol {e} with ord {ord(e)} in symbol_to_id_map {cleaned_text}")
      raise Exception(f"Error: {e} not found in symbol_to_id_map")
...
def get_bert(norm_text, word2ph, language, device):
    ...
    lang_bert_func_map = {"ZH": zh_bert, "EN": en_bert, "JP": jp_bert, 'ZH_MIX_EN': zh_mix_en_bert, 
                          'FR': fr_bert, 'SP': sp_bert, 'ES': sp_bert, "KR": kr_bert, "DE": de_bert}
